...
expected_monthly_btc_interest=8
yearly_funding=0.065
pct_funding_allocated_to_options=1
...
# --- Core simulation ---
def simulate_hedge_scenario_roll_yearly_call(
    btc_spot=btc_spot,
...
def simulate_hedge_scenario(
    ...
    In this scenario, we buy OTM call for the same strike each month, and for the same expiration:
    start date + 1y
    """
    ...
    # Option pricing
    hedge_df["strike_price_yearly_calls"] = hedge_df["btc_price"].iloc[0] + strike_premium # if we buy each month for the same strike 

    months_total = len(hedge_df)

    hedge_df["ttm_years"] = (months_total - hedge_df.index) / 12

    hedge_df["yearly_call_price"] = hedge_df.apply(
        lambda row: bs_call_price(
            row["btc_price"], 
            row["strike_price_yearly_calls"], 
            row["ttm_years"], 
            iv, 
            r
        ), 
    ...
    hedge_df.loc[hedge_df["yearly_call_price"] <= 10, 'num_yearly_calls'] = 0 # in our simulation for -50pct for example, last month spot is at 55k, strike is still 150k,call price doesnt make sense anymore
    hedge_df.loc[hedge_df["yearly_call_price"] <= 10, 'yearly_call_price'] = 0 # in our simulation for -50pct for example, last month spot is at 55k, strike is still 150k,call price doesnt make sense anymore

    hedge_df['additional_investment_to_fully_cover_upside'] = (expected_monthly_btc_interest - hedge_df['num_yearly_calls']) * hedge_df["yearly_call_price"]
    hedge_df.loc[hedge_df['num_yearly_calls'] > expected_monthly_btc_interest, 'additional_investment_to_fully_cover_upside'] = 0
    ...
    hedge_df['eoy_profit_from_calls'] = 0
    hedge_df['eoy_profit_from_calls'].iloc[-1] = (hedge_df['btc_price'].iloc[-1] - hedge_df["strike_price_yearly_calls"].iloc[-1]) * hedge_df['perp_position'].iloc[0]

#! todo: compute total cost additional investment to fully cover upside - eoy prift,
#! and look at dofference compared to what we would have lost whithout it in case of upside

    total_profit_with_hedge = (
        profit_without_hedge + profit_from_hedge
    )
...
if __name__ == '__main__':
    ...
    output_path = "/Users/annabellesoula/Documents/GitHub/hedge_simulation/btc_hedge_simulation_results_premium.xlsx"
